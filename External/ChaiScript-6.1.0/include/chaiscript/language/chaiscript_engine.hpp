...
namespace chaiscript
{
  ...
  class ChaiScript_Basic {
    ...
    std::function<std::string(const std::string &)> m_load_file_custom;
    ...
    std::string load_file(const std::string &t_filename) const {
      if (m_load_file_custom) { 
          try {
              return m_load_file_custom(t_filename);
          } catch (...) {
  ...
  public:
    ...
    ChaiScript_Basic(const ModulePtr &t_lib,
                     ...
                     std::function<std::string (const std::string &)> t_load_file_custom = {},
    ...
    explicit ChaiScript_Basic(std::unique_ptr<parser::ChaiScript_Parser_Base> &&parser,
                     ...
                     std::function<std::string (const std::string &)> t_load_file_custom = {},
                     const std::vector<chaiscript::Options> &t_opts = chaiscript::default_options())
      : ChaiScript_Basic({}, std::move(parser), t_module_paths, t_use_paths, t_load_file_custom, t_opts)
    {
      try {
