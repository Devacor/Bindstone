...
namespace MV{
	...
	FrameSwapper::FrameSwapper()
		:defaultDuration(0.1), previousFrame(0), framesListReference(0){
	}

	void FrameSwapper::setDefaultFrameDuration( Stopwatch::TimeType newDurationSeconds ){
		defaultDuration = newDurationSeconds;
	}
	...
	bool FrameSwapper::getCurrentFrame( Frame &a_returnFrame ){
		std::cout << "(" << timer.check() << ")" << std::endl;
		bool hasWrapped = false;
		if(framesListReference->frameList.empty()){
			std::cout << "EMPTY FRAME LIST REFERENCE" << std::endl;
	...
	Stopwatch::TimeType FrameSwapper::getNextFrameTime(){
		return (frame->duration == -1.0)?defaultDuration:frame->duration;
	}
