...
namespace MV{
	struct Frame{
		Frame(){duration = -1;}
		Frame(std::string a_mainTexture, std::string a_subTexture, Stopwatch::TimeType a_duration = -1.0):
			duration(a_duration),
			mainTexture(a_mainTexture),
			subTexture(a_subTexture){
		}
		Frame(boost::property_tree::ptree &frameInput){
			...
			std::cout << "Loading: " << mainTexture << "::" << subTexture << " Duration: " << duration << std::endl;
		}
	...
	public:
		...
		void setDefaultFrameDuration(Stopwatch::TimeType newDurationSeconds);
	...
	private:
		Stopwatch::TimeType getNextFrameTime();

		FrameSwapperDefinition *framesListReference;

		Stopwatch::TimeType defaultDuration;
		Stopwatch timer;
		Stopwatch::TimeType previousFrame, nextFrame;
