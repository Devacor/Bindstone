...
std::shared_ptr<MV::Scene::Button> makeSceneButton(const std::shared_ptr<MV::Scene::Node> &a_parent, MV::TextLibrary &a_library, MV::MouseState &a_mouse, const std::string &a_name, const MV::Size<> &a_size, const MV::UtfString &a_text, const std::string &a_fontIdentifier /*= MV::DEFAULT_ID*/) {
	...
	auto button = a_parent->make<MV::Scene::Button>(MV::wideToString(a_text) + boost::lexical_cast<std::string>(buttonId++), &a_mouse, a_size);
	auto activeScene = MV::Scene::Rectangle::make(a_parent->getRenderer(), a_size);
	colorTopAndBottom(activeScene, {InterfaceColors::SCENE_BUTTON_TOP_ACTIVE}, {InterfaceColors::SCENE_BUTTON_BOTTOM_ACTIVE});

	auto idleScene = MV::Scene::Rectangle::make(a_parent->getRenderer(), a_size);
	colorTopAndBottom(idleScene, {InterfaceColors::SCENE_BUTTON_TOP_IDLE}, {InterfaceColors::SCENE_BUTTON_BOTTOM_IDLE});

	auto activeBox = activeScene->make<MV::Scene::Text>("TextBox", &a_library, a_size - MV::size(10.0f, 0.0f), a_fontIdentifier)->
		position({5.0f, 0.0f})->
		justification(MV::TextJustification::LEFT)->
		wrapping(MV::TextWrapMethod::NONE)->setMinimumLineHeight(a_size.height)->text(a_text);

	auto idleBox = idleScene->make<MV::Scene::Text>("TextBox", &a_library, a_size - MV::size(10.0f, 0.0f), a_fontIdentifier)->
		position({5.0f, 0.0f})->
		justification(MV::TextJustification::LEFT)->
		wrapping(MV::TextWrapMethod::NONE)->setMinimumLineHeight(a_size.height)->text(a_text);
