...
namespace MV {
	...
	chaiscript::ChaiScript& Interface::hook(chaiscript::ChaiScript &a_script) {
		...
		a_script.add(chaiscript::fun(&Interface::node), "node");
	...
	void Interface::initialize() {
		if (!node) {
			node = MV::Scene::Node::load("Assets/UI/" + pageId + ".scene", JsonNodeLoadBinder(manager.managers(), manager.mouse(), manager.script()));

			scriptFileEval("Assets/UI/" + pageId + ".script", manager.script(), {
