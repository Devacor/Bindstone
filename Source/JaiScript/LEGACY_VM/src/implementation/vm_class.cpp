...
std::shared_ptr<vm_class_definition> vm_class_compiler::compile_class(class_decl* class_ast) {
    ...
    if (!class_ast->base_class_name.empty()) {
        auto eng = compiler_.get_vm()->get_engine_reference().lock();
        if (!eng) {
            throw std::runtime_error("Engine reference lost during class definition");
        }
        auto base = eng->get_class_registry().find_script_class(class_ast->base_class_name);
        if (!base) {
...
void vm_class_executor::execute_define_class(vm_context& ctx) {
    ...
    if (!base_class_name.empty()) {
        auto eng = compiler_.get_vm()->get_engine_reference().lock();
        if (!eng) {
            throw std::runtime_error("Engine reference lost during method compilation");
        }
        auto base = eng->get_class_registry().find_script_class(base_class_name);
        if (!base) {
...
void vm_class_executor::execute_end_class(vm_context& ctx) {
    ...
    // Register the class
    auto eng = executor_.get_engine_reference().lock();
    if (!eng) {
        throw std::runtime_error("Engine reference lost during class registration");
    }
    eng->get_class_registry().register_script_class(current_class_def_);
...
void vm_class_executor::execute_new_instance(vm_context& ctx) {
    ...
    auto eng = executor_.get_engine_reference().lock();
    if (!eng) {
        throw std::runtime_error("Engine reference lost during instance creation");
    }
    instance->class_def = eng->get_class_registry().find_script_class(class_name);
...
void vm_class_executor::execute_instanceof(vm_context& ctx) {
    ...
    if (value.is_object()) {
        ...
        if (instance) {
            auto eng = executor_.get_engine_reference().lock();
            if (!eng) {
                throw std::runtime_error("Engine reference lost during instanceof check");
            }
            result = eng->get_class_registry().is_assignable_from(instance->class_name, class_name);
        }
    }
...
void vm_class_executor::execute_checkcast(vm_context& ctx) {
    ...
    if (value.is_object()) {
        ...
        if (instance) {
            auto eng = executor_.get_engine_reference().lock();
            if (!eng) {
                throw std::runtime_error("Engine reference lost during cast");
            }
            if (!eng->get_class_registry().is_assignable_from(instance->class_name, target_class)) {
