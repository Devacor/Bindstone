...
JAI_TEST(exception_cleanup_stress) {
    ...
    engine.addFunction("allocate_data", []() -> String {
        // Allocate 1MB of data
        return String(std::string(1024 * 1024, 'X'));
    });
...
JAI_TEST(value_assignment_chain_leak) {
    ...
    for (int iter = 0; iter < 1000; ++iter) {
        Value v1 = String("Initial string that is reasonably long to ensure heap allocation");
        Value v2 = Int(42);
        Value v3 = Float(3.14159);
        Value v4 = Bool(true);
        
        // Create assignment chains
        v1 = v2 = v3 = v4 = String("New value for all");
        v4 = v3 = v2 = v1 = Int(999);
        v1 = v2 = String("Another string");
        v3 = v4 = Float(2.718);
        
        // Create temporary values
        for (int i = 0; i < 100; ++i) {
            Value temp = String("Temporary " + std::to_string(i));
            temp = Int(i);
            temp = String("Changed " + std::to_string(i));
        }
    }
...
JAI_TEST(function_binding_memory_leak) {
    ...
    for (int iter = 0; iter < 100; ++iter) {
        ...
        for (int i = 0; i < 100; ++i) {
            ...
            engine.addFunction(name, [captured_data, i]() -> String {
                return String(captured_data + " called");
            });
        }
