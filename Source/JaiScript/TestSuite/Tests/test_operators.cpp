...
JAI_TEST(compound_assignment_plus_equals) {
    ...
    engine.execute("x += 5;");
    expect_eq(engine.getVariable("x").as<int64_t>(), int64_t(15));
    
    // Test with floats
    engine.execute("float y = 10.5;");
    engine.execute("y += 2.5;");
    expect_eq(engine.getVariable("y").as<double>(), 13.0);
    
    // Test with strings
    engine.execute("string s = \"Hello\";");
    engine.execute("s += \" World\";");
    expect_eq(engine.getVariable("s").as<std::string>(), std::string("Hello World"));
}
...
JAI_TEST(compound_assignment_minus_equals) {
    ...
    engine.execute("int x = 10;");
    engine.execute("x -= 3;");
    expect_eq(engine.getVariable("x").as<int64_t>(), int64_t(7));
    
    engine.execute("float y = 10.5;");
    engine.execute("y -= 2.5;");
    expect_eq(engine.getVariable("y").as<double>(), 8.0);
}
...
JAI_TEST(compound_assignment_multiply_equals) {
    ...
    engine.execute("int x = 10;");
    engine.execute("x *= 3;");
    expect_eq(engine.getVariable("x").as<int>(), 30);
    
    engine.execute("float y = 2.5;");
    engine.execute("y *= 4.0;");
    expect_eq(engine.getVariable("y").as<double>(), 10.0);
}
...
JAI_TEST(compound_assignment_divide_equals) {
    ...
    engine.execute("x /= 4;");
    expect_eq(engine.getVariable("x").as<int>(), 5);
    
    engine.execute("float y = 10.0;");
    engine.execute("y /= 4.0;");
    expect_eq(engine.getVariable("y").as<double>(), 2.5);
}
...
JAI_TEST(prefix_increment) {
    ...
    expect_eq(engine.getVariable("x").as<int>(), 11);
...
JAI_TEST(postfix_increment) {
    ...
    expect_eq(engine.getVariable("x").as<int>(), 11);  // But variable is incremented
    
    engine.execute("float y = 1.5;");
    auto floatResult = engine.execute("y++");
    expect_eq(floatResult.as<double>(), 1.5);  // Should return old value
    expect_eq(engine.getVariable("y").as<double>(), 2.5);  // But variable is incremented
}
...
JAI_TEST(prefix_decrement) {
    ...
    expect_eq(engine.getVariable("x").as<int>(), 9);
    
    engine.execute("float y = 1.5;");
    auto floatResult = engine.execute("--y");
    expect_eq(floatResult.as<double>(), 0.5);
    expect_eq(engine.getVariable("y").as<double>(), 0.5);
}
...
JAI_TEST(postfix_decrement) {
    ...
    auto result = engine.execute("x--");
    expect_eq(result.as<int>(), 10);  // Should return old value
    expect_eq(engine.getVariable("x").as<int>(), 9);  // But variable is decremented
    
    engine.execute("float y = 1.5;");
    auto floatResult = engine.execute("y--");
    expect_eq(floatResult.as<double>(), 1.5);  // Should return old value
    expect_eq(engine.getVariable("y").as<double>(), 0.5);  // But variable is decremented
}
...
JAI_TEST(chained_compound_assignments) {
    ...
    expect_eq(engine.getVariable("x").as<int>(), 20);
    expect_eq(engine.getVariable("y").as<int>(), 10);
}
...
JAI_TEST(increment_in_expressions) {
    ...
    expect_eq(engine.getVariable("y").as<int>(), 12);
    expect_eq(engine.getVariable("x").as<int>(), 7);
}
...
JAI_TEST(compound_assignment_with_expressions) {
    ...
    expect_eq(engine.getVariable("x").as<int>(), 17);
}
...
JAI_TEST(logical_operators) {
    ...
    expect_eq(engine.getVariable("x").as<int>(), 0);  // x should not be modified
...
JAI_TEST(spaceship_operator_in_expressions) {
    ...
    expect_eq(engine.getVariable("result").as<int>(), -100);
}
