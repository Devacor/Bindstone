...
JAI_TEST(basic_operator_test) {
    ...
    makeClassBuilder<Vec>(engine, "Vec")
    ...
    // Simple operator
    engine.addFunction("+", [](const Vec& a, const Vec& b) -> Vec {
    ...
    try {
        ...
        if (engine.hasVariable("+")) {
            std::cout << "+ operator is registered\n";
            auto plusOp = engine.getVariable("+");
            if (plusOp.isFunction()) {
        ...
        Value result = engine.execute("v1 + v2;");
