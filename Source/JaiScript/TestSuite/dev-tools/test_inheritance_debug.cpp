...
int main() {
    try {
        ...
        Value result1 = engine.execute("auto c = Circle{5.0}; c;");
        std::cout << "Circle created successfully\n";
        
        std::cout << "\n=== Calling c.area() ===\n";
        Value result2 = engine.execute("auto c = Circle{5.0}; c.area();");
        std::cout << "area() result: " << result2.toString() << "\n";
        
        std::cout << "\n=== Calling c.type() ===\n";
        Value result3 = engine.execute("auto c = Circle{5.0}; c.type();");
        std::cout << "type() result: " << result3.toString() << "\n";
        
        std::cout << "\n=== Full test ===\n";
        Value result4 = engine.execute(R"(
            auto c = Circle{5.0};
            string shapeType = c.type();
            shapeType;
        )");
        std::cout << "Final result: " << result4.toString() << "\n";
