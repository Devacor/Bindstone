...
JAI_TEST(debug_vec2_creation) {
    ...
    try {
        Value result = engine.execute("var v = Vec2(3.0, 4.0); v;");
...
JAI_TEST(debug_operator_registration) {
    ...
    try {
        ...
        try {
            ...
            std::vector<Value> args = {v1_result, v2_result};
            
            // Try calling the overloaded function directly
            auto var = engine.getVariable("+");
            if (var.isFunction()) {
                std::cout << "Found + function, trying to call it...\n";
                try {
                    auto func = var.asFunction();
...
JAI_TEST(debug_type_info) {
    ...
    engine.addFunction("makeVec2", [](float x, float y) -> Vec2 {
        std::cout << "DEBUG: makeVec2(" << x << ", " << y << ") called\n";
        return Vec2(x, y);
    });
    
    try {
        Value result = engine.execute("var v = makeVec2(5.0, 6.0); v;");
        auto v = result.as<std::shared_ptr<Vec2>>();
