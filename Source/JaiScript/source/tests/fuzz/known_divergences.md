...
| KD-ENV-RECURSION-LIMIT | deep-but-legal recursion (under `JAI_MAX_CALL_DEPTH` native frames, e.g. ~7.3k): interpreter's ENVIRONMENT recursion guard errors first; the vm's in-loop frames complete it and continue (erroring later or succeeding). CHARTER WIDENED (Dev ruling, TCO landed 30d5c9cd): bare self tail calls (`return f(args)`, no enclosing try) now splice frames on the vm — tail recursion of ANY depth completes (or runs to the execution budget) where the interpreter raises `max_recursion_depth` at 10k. Same judgment, same suppression. | 2072 (needs `--timeout-ms 5000`; at the default 250ms Debug budget it hides as a budget-skip) | interpreter (the depth was legal; the env guard fires below the documented cap; for tail shapes the vm's unbounded depth is the DESIGNED behavior) | ACCEPTED divergence (Dev ruling 2026-07-09; tail charter 2026-07-14): closing it means refactoring the tree walker off native recursion — not needed; parked as a low-priority item. Suppression is permanent. |
...
|----|-----------|------------|
| KD-CORO-RESUME-PAST-END | fixed 2026-07-11: the interpreter's `resume_coroutine` replay-completion branch never updated `handle.yield_value_` when the body ran past the final `yield` with no `return`, so the resume replayed the STALE last-yielded value (vm `run_fiber` correctly completes with null). Second shape in the same family: a zero-yield coroutine completing on its FIRST resume went through `call_function`, whose no-return epilogue applied the implicit-'this' method rule — a METHOD coroutine returned its receiver instead of null. Both interpreter paths now complete with null (explicit returns unaffected); recorded campaign seeds 4 38 98 195 216 387 626 840 1013 1099 1482 1729 2047 2049 2064 2076 2261 2483 2496 2990 (plus pilot 7, 116) re-verified agreeing with `--no-suppressions`. Test: `fuzz_coro_resume_past_end_yields_null` (review_regression_tests.cpp). | (this branch) |
