...
std::vector<declaration_ptr> parse(const std::string& code) {
    lexer lex(code, "test.jai");
    auto tokens = lex.tokenize();
    parser p(tokens, "test.jai");
    return p.parse();
}

// Helper to check parsing succeeds
void assertParseSucceeds(const std::string& code) {
    jai::lexer lex(code, "test.jai");
    auto tokens = lex.tokenize();
    jai::parser p(tokens, "test.jai");
    auto ast = p.parse();
...
JAI_TEST(constructor_calls) {
    assertParseSucceeds("MyClass();");
    assertParseSucceeds("Point(x, y);");
    // array() without type parameters should fail since array is a keyword requiring <T>
    assertParseFails("array();");  // Expected: array<int>() or similar
}
