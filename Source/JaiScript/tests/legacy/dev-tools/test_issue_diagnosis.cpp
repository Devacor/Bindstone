...
JAI_TEST(lambda_basic_execution) {
    ...
    try {
        ...
        std::cout << "Lambda result: " << result.to_string() << std::endl;
...
JAI_TEST(control_flow_for_loop) {
    ...
    try {
        ...
        std::cout << "For loop result: " << result.to_string() << std::endl;
...
JAI_TEST(control_flow_while_loop) {
    ...
    try {
        ...
        std::cout << "While loop result: " << result.to_string() << std::endl;
        expect_eq(result.as<Int>(), 10);
    } catch (const std::exception& e) {
...
JAI_TEST(control_flow_if_else) {
    ...
    try {
        ...
        std::cout << "If-else result: " << result.to_string() << std::endl;
...
JAI_TEST(string_concatenation_in_loop) {
    ...
    engine.add_function("make_string", [](int size) -> String {
...
JAI_TEST(function_call_in_expression) {
    ...
    try {
        ...
        std::cout << "Function call result: " << result.to_string() << std::endl;
        expect_eq(result.as<Int>(), 10);
    } catch (const std::exception& e) {
