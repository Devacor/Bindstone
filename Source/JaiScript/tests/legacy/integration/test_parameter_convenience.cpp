...
JAI_TEST(function_parameter_styles) {
    ...
    engine.add_function("scaleByValue", [](const Circle& c, float factor) -> Circle {
        std::cout << "scaleByscript_value called with radius=" << c.radius << ", factor=" << factor << " (const ref - no copy)\n";
        return Circle(c.radius * factor);
    });
    
    // Style 2: By shared_ptr parameter (explicit, no copy)
    engine.add_function("scaleByPtr", [](std::shared_ptr<Circle> c, float factor) -> Circle {
...
JAI_TEST(mixed_parameter_styles) {
    ...
    engine.add_function("combine", [](const Circle& c1, std::shared_ptr<Circle> c2) -> float {
        std::cout << "combine called: r1=" << c1.radius << ", r2=" << c2->radius << "\n";
        return c1.radius + c2->radius;
    });
    
    // Test shared_ptr reference parameters
    engine.add_function("distance", [](std::shared_ptr<Circle>& c1, std::shared_ptr<Circle>& c2) -> float {
    ...
    engine.add_function("modifyRadius", [](Circle& c, float newRadius) -> void {
        std::cout << "modifyRadius called: old=" << c.radius << ", new=" << newRadius << " (Circle& - no copy)\n";
        c.radius = newRadius;
    });
    
    // Test shared_ptr reference (std::shared_ptr<Circle>&)
    engine.add_function("swapCircles", [](std::shared_ptr<Circle>& c1, std::shared_ptr<Circle>& c2) -> void {
    ...
    script_value distResult = engine.execute(R"(
        var c1 = Circle(5.0);
        var c2 = Circle(2.0);
        distance(c1, c2);
    )");
