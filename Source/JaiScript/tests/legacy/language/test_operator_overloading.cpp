...
JAI_TEST(basic_operator_overloading_for_custom_type) {
    ...
    engine.add_function("+", [](const Vector2& a, const Vector2& b) -> Vector2 {
    ...
    engine.add_function("*", [](const Vector2& v, float scalar) -> Vector2 {
        return Vector2(v.x * scalar, v.y * scalar);
    });
    
    engine.add_function("*", [](float scalar, const Vector2& v) -> Vector2 {
        return Vector2(v.x * scalar, v.y * scalar);
    });
...
JAI_TEST(comparison_operator_overloading) {
    ...
    engine.add_function("==", [](const Vector2& a, const Vector2& b) -> bool {
...
JAI_TEST(operator_overloading_with_multiple_types) {
    ...
    engine.add_function("+", [](const Money& a, const Money& b) -> Money {
    ...
    engine.add_function("*", [](const Money& m, const Percentage& p) -> Money {
        return Money(m.amount * (p.value / 100.0f));
    });
    
    // Money + Money from percentage calculation
    engine.add_function("+", [](const Money& m, float amount) -> Money {
...
JAI_TEST(operator_precedence_with_overloading) {
    ...
    // script_string operators with custom behavior
    engine.add_function("+", [](const std::string& a, const std::string& b) -> std::string {
        return a + b;
    });
    
    // script_string * int = repeat string
    engine.add_function("*", [](const std::string& s, int n) -> std::string {
        std::string result;
        for (int i = 0; i < n; i++) {
            result += s;
        }
...
JAI_TEST(chained_operator_overloading) {
    ...
    engine.add_function("*", [](const Vector2& v, float s) -> Vector2 {
...
JAI_TEST(operator_overloading_error_handling) {
    ...
    engine.add_function("/", [](float a, float b) -> float {
    ...
    script_value result = engine.execute("10.0 / 2.0;");
...
JAI_TEST(mixed_type_arithmetic_overloading) {
    ...
    // Complex + Complex
    engine.add_function("+", [](const Complex& a, const Complex& b) -> Complex {
        return Complex(a.real + b.real, a.imag + b.imag);
    });
    
    // Complex + float (real number)
    engine.add_function("+", [](const Complex& c, float r) -> Complex {
        return Complex(c.real + r, c.imag);
    });
    
    // float + Complex
    engine.add_function("+", [](float r, const Complex& c) -> Complex {
        return Complex(c.real + r, c.imag);
    });
