...
JAI_TEST_SUITE(MemoryStressTests)
    ...
    JAI_TEST(repeated_cpp_function_binding) {
        runMemoryTest("C++ function binding", 100, []() {
            ...
            for (int i = 0; i < 50; ++i) {
                ...
                engine.add_function(name, [i](int x) -> int { return x + i; });
            }
    ...
    JAI_TEST(repeated_class_binding) {
        runMemoryTest("Class binding", 100, []() {
            ...
            using Builder = class_builder<std::vector<int>>;
    ...
    JAI_TEST(complex_memory_stress_scenario) {
        ...
        runMemoryTest("Complex scenario with potential leaks", 50, []() {
            ...
            engine.add_function("create_array", []() -> std::vector<int> {
                return std::vector<int>{1, 2, 3, 4, 5};
            });
            
            engine.add_function("process_array", [](const std::vector<int>& arr) -> int {
                int sum = 0;
                for (int x : arr) sum += x;
                return sum;
            });
    ...
    JAI_TEST(global_state_accumulation) {
        ...
        for (int iter = 0; iter < 100; ++iter) {
            ...
            for (int i = 0; i < 100; ++i) {
                ...
                engine.add_global(name, script_int(i));
            }
            
            // Access some globals
            script_value result = engine.execute("global_0 + global_50 + global_99;");
            expect_eq(result.as<script_int>(), 149);
        }
    }
}
