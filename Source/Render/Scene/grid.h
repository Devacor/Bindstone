...
namespace MV {
	namespace Scene {
		...
		private:
			...
			virtual bool handleBegin(std::shared_ptr<VisualChange> a_change) override{
				dirtyGrid = dirtyGrid || a_change->changeShape;
				return true;
			}
			virtual void handleEnd(std::shared_ptr<VisualChange>) override{
			}

			template <class Archive>
			void serialize(Archive & archive){
				archive(CEREAL_NVP(maximumWidth),
			...
			static void load_and_construct(Archive & archive, cereal::construct<Grid> &construct){
				...
				construct(renderer);
				archive(cereal::make_nvp("maximumWidth", construct->maximumWidth),
			...
			virtual void onChildAdded(std::shared_ptr<Node>) override{
				dirtyGrid = true;
			}
			void layoutCellSize();
