...
static void MESA__gluMultMatrixVecd(const GLdouble matrix[16], const GLdouble in[4], GLdouble out[4]) {
   int i;
...
GLint GLAPI MESA_gluProject(GLdouble objx, GLdouble objy, GLdouble objz, const GLdouble modelMatrix[16], const GLdouble projMatrix[16], const GLint viewport[4], GLdouble *winx, GLdouble *winy, GLdouble *winz) {
      double in[4];
      double out[4];
      ...
      in[2]=objz;
      in[3]=1.0;
      MESA__gluMultMatrixVecd(modelMatrix, in, out);
      MESA__gluMultMatrixVecd(projMatrix, out, in);
...
namespace MV {
   ...
   glExtensionBlendMode::glExtensionBlendMode() :initialized(false),pglBlendFuncSeparateEXT(nullptr){
   }
   ...
   void glExtensionBlendMode::loadExtensionBlendMode( char *a_extensionsList ){
      if(pglBlendFuncSeparateEXT==nullptr){
   ...
   glExtensionFramebufferObject::glExtensionFramebufferObject(int &a_windowWidth, int &a_windowHeight) 
      ...
      initialized(false),pglIsRenderbufferEXT(nullptr),pglBindRenderbufferEXT(nullptr),pglDeleteRenderbuffersEXT(nullptr),
   ...
   void glExtensionFramebufferObject::loadExtensionFramebufferObject( char* a_extensionsList ){
      if(pglGenFramebuffersEXT==nullptr){
   ...
   Point Draw2D::getObjectToWorldPoint( double a_worldX, double a_worldY ){
      ...
      MESA_gluProject(a_worldX, a_worldY, 0.0, &(*modelviewMatrix().top().getMatrixArray())[0], &(*projectionMatrix().top().getMatrixArray())[0], viewport, &tmpPoint.x, &tmpPoint.y, &tmpPoint.z);
   ...
   Point Draw2D::getWindowToWorldPoint( double a_windowX, double a_windowY ){
      ...
      MESA_gluProject(a_windowX, a_windowY, 0.0, &(*modelviewMatrix().top().getMatrixArray())[0], &(*projectionMatrix().top().getMatrixArray())[0], viewport, &tmpPoint.x, &tmpPoint.y, &tmpPoint.z);
      //Re-orient for our actual window position based on 0, 0 as top left instead of bottom left.
      tmpPoint.y-=windowHeight;
      tmpPoint.y*=-1.0;
      //Finally, retrieve the actual location of the object in the world by taking into account the window dimensions
