...
public:
	...
	DiggerWorld(const std::shared_ptr<MV::Scene::Node> &a_node, MV::SharedTextures &a_sharedTextures, MV::TapDevice &a_mouse) :
		...
		physicsWorld(a_node->make("Physics")->attach<MV::Scene::Environment>(MV::Point<>(0.0f, 100.0f))),
		mouse(a_mouse),
		sharedTextures(a_sharedTextures){
		...
		auto bodySize = MV::BoxAABB<>{ MV::size(25.0f, 38.0f), true };
		auto litterSize = MV::BoxAABB<>{ MV::size(10.0f, 10.0f), true };
		auto thingSize = MV::BoxAABB<>{ MV::size(20.0f, 20.0f), true };
		thing = physicsWorld->owner()->make("thing")->position({ 100.0f, 5.0f })->
			attach<MV::Scene::Sprite>()->bounds(thingSize)->color({ 0.0f, 0.0f, 1.0f, .9f })->owner()->
			attach<MV::Scene::Collider>(physicsWorld, MV::Scene::CollisionBodyAttributes().makeDynamic().angularDamping(55.0f));
		physicsWorld->owner()->make("thing1")->position({ 100.0f, -50.0f })->
			attach<MV::Scene::Sprite>()->bounds(litterSize)->color({ 0.0f, 1.0f, 1.0f, .5f })->owner()->
			attach<MV::Scene::Collider>(physicsWorld, MV::Scene::CollisionBodyAttributes().makeDynamic())->attach(litterSize)->id("thing1");
		physicsWorld->owner()->make("thing2")->position({ 150.0f, 30.0f })->
			attach<MV::Scene::Sprite>()->bounds(litterSize)->color({ 0.0f, 1.0f, 1.0f, .5f })->owner()->
			attach<MV::Scene::Collider>(physicsWorld, MV::Scene::CollisionBodyAttributes().makeDynamic())->attach(litterSize)->id("thing2");
		physicsWorld->owner()->make("thing3")->position({ 140.0f, 10.0f })->
			attach<MV::Scene::Sprite>()->bounds(litterSize)->color({ 0.0f, 1.0f, 1.0f, .5f })->owner()->
			attach<MV::Scene::Collider>(physicsWorld, MV::Scene::CollisionBodyAttributes().makeDynamic())->attach(litterSize)->id("thing3");
		
		thing->attach(20.0f, MV::Point<>(), MV::Scene::CollisionPartAttributes().id("foot").friction(15.0f).restitution(0));

		auto topBody = physicsWorld->owner()->make("thingBody")->position({ 100.0f, -12.0f })->
			attach<MV::Scene::Sprite>()->bounds(bodySize)->color({ 0.0f, 1.0f, 0.0f, .6f })->owner()->
			attach<MV::Scene::Collider>(physicsWorld, MV::Scene::CollisionBodyAttributes().makeDynamic().disableRotation());
		topBody->attach(bodySize, 0.0f, MV::Scene::CollisionPartAttributes().friction(0.0f).restitution(0));
	...
	void pushTile(bool a_collidable, const std::shared_ptr<MV::TextureHandle> &a_environment, const std::shared_ptr<MV::TextureHandle> &a_background, const std::shared_ptr<MV::TextureHandle> &a_foreground = nullptr) {
		...
		if (a_environment) {
			worldTile = environment->owner()->make()->attach<MV::Scene::Clickable>(mouse)->bounds({ tileSize, true })->show()->texture(a_environment)->owner();
		} else {
			worldTile = environment->owner()->make()->attach<MV::Scene::Clickable>(mouse)->bounds({ tileSize, true })->show()->color({ 0, 0, 0, 0 })->owner();
		}
		if (a_background) {
			background->owner()->make()->attach<MV::Scene::Sprite>()->bounds({ tileSize, true })->texture(a_background);
		} else {
			background->owner()->make()->attach<MV::Scene::Sprite>()->bounds({ tileSize, true })->color({ 0, 0, 0, 0 });
		}
		if (a_foreground) {
			foreground->owner()->make()->attach<MV::Scene::Sprite>()->bounds({ tileSize, true })->texture(a_foreground);
		} else {
			foreground->owner()->make()->attach<MV::Scene::Sprite>()->bounds({ tileSize, true })->color({ 0, 0, 0, 0 });
		}

		if (a_collidable) {
			environment->layoutCells();
			worldTile->attach<MV::Scene::Collider>(physicsWorld, MV::Scene::CollisionBodyAttributes().makeStatic())->
				attach(MV::BoxAABB<>{tileSize, true})->
				id("T:" + std::to_string(tileCounter));
